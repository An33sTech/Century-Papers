...
if ($_SERVER['HTTP_HOST'] == 'localhost' || $_SERVER['REMOTE_ADDR'] == '192.168.10.5') {
...
$sqli  = "SELECT * FROM product_cache_image WHERE image = ? AND weight = ? AND height = ? AND img_cache = ? ";

$arrayi  =    array($image, $maxWidth, $maxHeight, $resizedImage);

$data = $dbF->getRow($sqli, $arrayi);
if (!$dbF->rowCount > 0) {

    $sql    =  "INSERT INTO product_cache_image (image,weight,height,img_cache)
VALUES (?,?,?,?)";
    $array  =    array($image, $maxWidth, $maxHeight, $resizedImage);
