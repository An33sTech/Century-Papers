...
class menu extends object_class
{
    ...
    public function mobiflexMyMenule_menu($mailUlId = '', $mainUlClass = '', $liClass = '', $aClass = '', $ulSubMenuClass = '', $liSubMenuClass = '', $aSubMenuClass = '')
    {
        ...
        foreach ($data as $val) {
            ...
            echo "<li class='$liClass' id='$id'><a href='$link' data-page='$link' title='$heading - Sharkspeed' class='$aClass'>$heading</a>
...
HTML;
    ...
    public function main_menu($type = 'main', $responsive = false, $mainUlId = '', $mainUlClass = '', $liClass = '', $aClass = '', $ulSubMenuClass = '', $liSubMenuClass = '', $aSubMenuClass = '')
    {
        ...
        foreach ($data as $val) {
            ...
            echo "<li class='$liClass $active $hasSubMenu menuFirstLi' id='$id'>
                        ...
                        <a href='$link' data-page='$link' title='$heading - Sharkspeed' class='$aClass' $icon><span>$heading</span> $dropCaret</a>
    ...
    public function main_menuFindSubMenu($id, $ulSubMenuClass, $liSubMenuClass, $aSubMenuClass, $heading = '', $main_menu_icon = '')
    {
        ...
        foreach ($data as $val) {
            ...
            $temp .= "<li class='$liSubMenuClass $active $hasSubMenu2' id='$id'><a href='$link' data-page='$link' title='$heading - Sharkspeed' class='$aSubMenuClass'>$heading $dropCaret</a>
    ...
    public function main_menuFindSubMenu2($id, $ulSubMenuClass, $liSubMenuClass, $aSubMenuClass)
    {
        ...
        foreach ($data as $val) {
            ...
            $temp .= "<li class='$liSubMenuClass $active' id='$id'><a href='$link' data-page='$link' title='$heading - Sharkspeed' class='$aSubMenuClass'>$heading</a></li>";
        }
    ...
    public function menuTypeSingle($type = 'main', $under = '0', $url_function = false)
    {
        ...
        foreach ($data as $val) {
            ...
            if (stripos($link, WEB_URL) === false) {
                $link = str_replace("https://sharkspeed.se", WEB_URL, $link);
            }
    ...
    public function footerAllMenu()
    {
        ...
        foreach ($footerMenu as $val) {
            ...
            $FooterLink = translateFromSerialize($val['link']);
            ...
            foreach ($footerMenu2 as $val2) {
                ...
                $FooterLink = translateFromSerialize($val2['link']);

                echo '<li><a href="' . $FooterLink . '" data-page="' . $FooterLink . '" title="' . $text . ' - Sharkspeed" class="">' . $text . '</a></li>';
            }
    ...
    public function FindSubcategory($id)
    {
        ...
        if (!$this->dbF->rowCount) {
            ...
            if ($data['under'] == 1) {
                ...
                // $sql_children_heading  = "SELECT * FROM `categories` WHERE `id` = ? ";
                // $data_children_heading = $this->dbF->getRow($sql_top,array($data['id']));

                $heading = translateFromSerialize($data_top['name']);
                $iconn = translateFromSerialize($data_top['icon']);

                // $heading = translateFromSerialize($data_children_heading['name']);
                // $iconn = translateFromSerialize($data_children_heading['icon']);
            ...
            foreach ($data_children as $inner_menu) {
                ...
                if ($pageC == $current_page) {
                    ...
                    $result_array['inner_lis'] .= '<a href="' . $link . '" data-page="' . $link . '" title="' . translateFromSerialize($inner_menu['name']) . ' - Sharkspeed" ' . $css . '>' . translateFromSerialize($inner_menu['name']) . '</a>';
                } else {
                    $link = $inner_menu['id'];
                    $result_array['inner_lis'] .= '<a href="' . $link . '" data-page="' . $link . '" title="' . translateFromSerialize($inner_menu['name']) . ' - Sharkspeed">' . translateFromSerialize($inner_menu['name']) . '</a>';
                }
            }
    ...
    public function FindSubcategoryTest($id)
    {
        ...
        if (!$this->dbF->rowCount) {
            ...
            if ($data['under'] == 1) {
                ...
                $data_children_heading = $this->dbF->getRow($sql_top, array($data['id']));
                $heading = translateFromSerialize($data_children_heading['name']);
                $icon = translateFromSerialize($data_children_heading['icon']);
            ...
            foreach ($data_children as $inner_menu) {
                ...
                $sql8 = "SELECT `slug`,`id` FROM `seo` WHERE `pageLink` = '$pLink'";
                ...
                if ($pageC == $current_page) {
                    $link = ($slug8 == NULL) ? WEB_URL . '/' . $inner_menu['id'] : WEB_URL . '/' . $slug8;
                    $result_array['inner_lis'] .= '<li class="active4"><i class="fa fa-arrow-right" aria-hidden="true"></i><a href="' . $link . '" data-category="' . $heading . '" data-type="subcategory" data-page="' . $link . '" title="' . translateFromSerialize($inner_menu['name']) . ' - Sharkspeed">' . translateFromSerialize($inner_menu['name']) . '</a></li>';
                } else {
                    // if(sizeof($data8) > 0){
                    //     $link = $slug8;
                    // }
                    ...
                    // $link   = $inner_menu['id'];
                    $link = ($slug8 == NULL) ? WEB_URL . '/' . $inner_menu['id'] : WEB_URL . '/' . $slug8;
                    $result_array['inner_lis'] .= '<li><i class="fa fa-arrow-right" id="1" aria-hidden="true"></i><a href="' . $link . '" data-category="' . $heading . '" data-type="subcategory" data-page="' . $link . '" title="' . translateFromSerialize($inner_menu['name']) . ' - Sharkspeed">' . translateFromSerialize($inner_menu['name']) . '</a></li>';
                }
            }
