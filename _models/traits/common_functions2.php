...
function data_compress($string, $base64 = true)
{
    ...
    if ($length1 > 495) {
        ...
        if ($base64)
            $compressed2 = base64_encode($compressed2);
...
function data_unCompress($string, $base64 = true)
{
    ...
    $unCompress = $string;
    if ($base64)
        $unCompress = base64_decode($unCompress);
...
trait common_function2
{
    ...
    public function uploadSingleImage($data, $folder = 'mixed', $imgName = '', $fileType = '', $path = '')
    {
        ...
        if ($this->check_file_allow($data["name"]) == false)
            return false;
        ...
        if (($data["size"]) > 0) {
            ...
            if (
                ($data["type"] == "image/png") || ($data["type"] == "image/jpeg") || ($data["type"] == "video/mp4") ||
    ...
    public function uploadMultiImages($data, $folder = 'mixed', $imgName = '', $fileType = '')
    {
        ...
        foreach ($dataTemp["size"] as $key => $val) {
            ...
            if (($data["size"][$key]) > 0) {
                if (
                    ($data["type"][$key] == "image/png") || ($data["type"][$key] == "image/jpeg") ||
                    ...
                    if ($data["error"][$key] > 0) {
                        ...
                        if ($this->check_file_allow($realName) == false)
                            continue;
    ...
    public function require_once_custom($fileName)
    {
        ...
        switch ($fileName) {
            ...
            case 'orderOrder':
                return require_once($adminFile . 'order/classes/order.php');
                break;
            case 'webUsers.class':
