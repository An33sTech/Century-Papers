...
class stock_ajax extends object_class{
    ...
    public  function directQTYAdd(){
        ...
        if($this->dbF->rowCount>0){
            ...
            $this->dbF->setRow($sql, array($pqty, $date, $hash));
        }else{
    ...
    public function directStockQTYAdd(){
        ...
        $sql    =   "UPDATE product_inventory SET qty_item = qty_item + ? WHERE qty_pk = ?";
        $this->dbF->setRow($sql, array($pqty, $id));
        if($this->dbF->rowCount>0){
            echo '1';
            $desc= _replace('{{qty}}',$pqty,$_e['Product Add QTY {{qty}}']);
    ...
    public function directStockLocationAdd(){
        ...
        $sql    =   "UPDATE product_inventory SET location = ?  WHERE qty_pk = ?";
        $this->dbF->setRow($sql,array($location, $id));
        if($this->dbF->rowCount>0){
            echo '1';
