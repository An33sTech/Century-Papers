...
if (1 === 2) {
    ...
    * */
            ...
            $(function () {
                $(obj).on("click", function () {
                    ...
                    if ($(this).attr("data-window") !== undefined) {
                        var formBody = '<form action="' + form_action +
                    ...
                    for (var i = 0; i < str_array.length; i++) {
                        ...
                        for (var i2 = 0; i2 < str_array_2.length; i2 = i2 + 2) {
                            formBody += '<input type="hidden" name="' + str_array_2[i2] +
                                '" value="' + str_array_2[(i2 + 1)] + '" />';
                        }
                    }
                    formBody += '</form>';
    ...
    functions = {
        ...
        CRC32: function (str) {
            var CRCTable = [0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535,
                0x9E6495A3,
                ...
                0xD56041E4,
                0xA2677172, 0x3C03E4D1, 0x4B04D447, 0xD20D85FD, 0xA50AB56B, 0x35B5A8FA, 0x42B2986C, 0xDBBBC9D6,
                0xACBCF940,
                ...
                0xAD678846,
                0xDA60B8D0, 0x44042D73, 0x33031DE5, 0xAA0A4C5F, 0xDD0D7CC9, 0x5005713C, 0x270241AA, 0xBE0B1010,
                0xC90C2086,
                ...
                0xAF0A1B4C,
                0x36034AF6, 0x41047A60, 0xDF60EFC3, 0xA867DF55, 0x316E8EEF, 0x4669BE79, 0xCB61B38C, 0xBC66831A,
                0x256FD2A0,
                ...
                0x6FB077E1,
                0x18B74777, 0x88085AE6, 0xFF0F6A70, 0x66063BCA, 0x11010B5C, 0x8F659EFF, 0xF862AE69, 0x616BFFD3,
                0x166CCF45,
    ...
    $(document).ready(function () {
        $(function () {
            ...
            $('.dTable tbody, .dTable_ajax tbody, .dTable_ajax1 tbody, .dTable_ajaxOrder tbody').on('click',
                'tr',
                function () {
                    fc.anchorByPost("a[data-method='post']");
                });
            ...
            $(".dTable tbody, .dTable_ajax tbody, .dTable_ajax1 tbody, .dTable_ajaxOrder tbody").on("click",
                ...
                function () {
                    ...
                    for (var i = 0; i < str_array.length; i++) {
                        ...
                        var str_array_2 = x2str.split('=');
                        for (var i2 = 0; i2 < str_array_2.length; i2 = i2 + 2) {
                            formBody += '<input type="hidden" name="' + str_array_2[i2] + '" value="' +
                                str_array_2[(i2 + 1)] + '" />';
                        }
                    }
                    formBody += '</form>';
    ...
    $(document).ready(function () {
        ...
        var dTableFull = $('.dTableFull').DataTable({
            aaSorting: [],
            aLengthMenu: [
                [5, 10, 25, 50, 100, 250, 500, 1000, 5000, -1],
                [5, 10, 25, 50, 100, 250, 500, 1000, 5000, "All"]
            ],
        ...
        table = $('.dTable').DataTable({
            <?php echo $aLengthMenu; ?> "sDom": 'T C<"clearfix">lfrtip',
            "columnDefs": [{
                "targets": [2],
                "visible": false
            },
        ...
        dTableT = function () {
            $('.dTableT').DataTable({
                <?php echo $aLengthMenu; ?> "sDom": 'T<"clearfix">lfrtip',
                <?php echo $oTableTools; ?>
            <?php echo $dataTable_language; ?>
            stateSave: true
            });
        ...
        dateCodeFrom = (typeof dateCodeFrom === 'undefined') ? console.log('dateCodeFrom is undefined') :
            dateCodeFrom;
        dateCodeTo = (typeof dateCodeTo === 'undefined') ? console.log('dateCodeTo is undefined') : dateCodeTo;

        $('.dTable_ajax').each(function (index) {
            var url1 = $(this).attr("data-href");
            var uniq = $(this).attr("data-uniq");
            var cols1 = ('undefined' === typeof $(this).attr("data-cols")) ? false : $(this).attr("data-cols");
            var sorting = ('undefined' === typeof $(this).attr("data-sorting")) ? false : $(this).attr("data-sorting");

            // Add uniq to URL
            url1 = url1 + "&uniq=" + uniq;
            // console.log("DataTable URL: " + url1);

            var columnDefsSixColumns1 = [
                ...
                { "targets": [-1], className: 'tableBgGray custom_width' }
            ];

            var columnDefsSevenColumns1 = [
                { "targets": [0], "orderable": true },
                { "targets": [1], "orderable": true },
                { "targets": [2], "visible": false, "orderable": true },
                { "targets": [3], "visible": false, "orderable": true },
                { "targets": [4], "orderable": true },
                { "targets": [5], "orderable": true },
                { "targets": [6], "orderable": true },
                { "targets": [-1], "orderable": false, className: 'tableBgGray custom_width' }
            ];

            // Determine which columnDefs to use
            var columnDefs1 = cols1 == 6 ? columnDefsSixColumns1 : columnDefsSevenColumns1;
            var bSortValue = (sorting === 'false' || sorting === false) ? false : true;

            var OrdersTable1 = $(this).DataTable({
                "lengthMenu": [5, 10, 25, 50, 100, 250, 500, 1000, 5000, "All"],
                "sDom": 'T C<"clearfix">lfrtip',
                "bSort": bSortValue,
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.selectedCountry = $("select[name='country']").val();
                        d.selectedInvoice = $("select[name='invoice_status']").val();
                        d.catego = (typeof joinTest === 'undefined') ? '' : joinTest;
                        d.selectedCheck = (typeof joinTest1 === 'undefined') ? '' : joinTest1;
                    }
                }
            });

            // Handle manual search logic
            if ($(this).closest('.search_manually').length > 0) {
                var dataTableSearchBox1 = $(this).closest('.dataTables_wrapper').find('.dataTables_filter input');
                dataTableSearchBox1.off();

                var searchBtnID1 = 'searchBtn1_' + index;
                $(this).closest('.dataTables_wrapper').find('#' + searchBtnID1).remove();

                $('<button id="' + searchBtnID1 + '" class="search_manually_btn form-control-feedback"><span class="glyphicon glyphicon-search"></span></button>')
                    .insertAfter(dataTableSearchBox1);

                $('#' + searchBtnID1).on('click', function () {
                    var searchTerm1 = dataTableSearchBox1.val();
                    OrdersTable1.search(searchTerm1).draw();
                });

                // Trigger search when pressing 'Enter' key in the search input
                dataTableSearchBox1.on('keydown', function (e) {
                    if (e.key === 'Enter') { // Check if 'Enter' key is pressed
                        var searchTerm1 = dataTableSearchBox1.val();
                        OrdersTable1.search(searchTerm1).draw(); // Trigger search only for the current table
                    }
                });
            }

            OrdersTable1.on('xhr', function () {
                if (typeof window.orderPrice === 'function') {
                    orderPrice();
                } else if (typeof window.reloadTable === 'function') {
                    reloadTable();
                } else {
                    console.log('No reload or price function available');
                }
            });
        });


        $('.dTable_ajax_lang').each(function (index) {
            ...
            // Show Entries Values
            // "lengthMenu": [ 25, 50, 100, 250, 500, 1000, 5000 ],
            "sDom": 'T C<"clearfix">frti',
                "bSort": false, // this will disable sorting all around, "orderable" inside "columnDefs" won't work 
                "columnDefs": [{
                    "targets": [0],
                    "orderable": false
                },
                ...
                <?php //echo $oTableTools; 
                ?>
            <?php echo $dataTable_language; ?> "bJQueryUI": false,
                "bAutoWidth": false,
            ...
            // Check if the specific container with unique ID exists for this DataTable
            if ($(this).closest('.search_manually').length > 0) {
                // Disable auto search on typing in the search input
                var dataTableSearchBox = $(this).closest('.dataTables_wrapper').find('.dataTables_filter input');
                dataTableSearchBox.off();

                // Create a unique ID for the search button based on the table index
                var searchBtnID = 'searchBtn2_' + index;

                // Remove any existing search buttons to avoid duplication
                $(this).closest('.dataTables_wrapper').find('#' + searchBtnID).remove();

                // Insert the search button dynamically after the search input with a unique ID
                $('<button id="' + searchBtnID + '" class="search_manually_btn form-control-feedback"><span class="glyphicon glyphicon-search"></span></button>').insertAfter(dataTableSearchBox);

                // Trigger search manually on button click only for this specific table
                $('#' + searchBtnID).on('click', function () {
                    var searchTerm = dataTableSearchBox.val();
                    OrdersTable.search(searchTerm).draw(); // Trigger search only for the current table
                });
                // Trigger search when pressing 'Enter' key in the search input
                dataTableSearchBox.on('keydown', function (e) {
                    if (e.key === 'Enter') { // Check if 'Enter' key is pressed
                        var searchTerm = dataTableSearchBox.val();
                        OrdersTable.search(searchTerm).draw(); // Trigger search only for the current table
                    }
        ...
        $('.dTable_ajax1').each(function (index) {
            url = $(this).attr("data-href");
            uniq = $(this).attr("data-uniq");
            ...
            sorting = ('undefined' === typeof $(this).attr("data-sorting")) ? false : $(this).attr(
                "data-sorting");
            <?php # add uniq as $_GET param on the current link 
            ?>
            url = url + "&uniq=" + uniq;
            ...
            OrdersTable = $(this).DataTable({
                ...
                ?>
            // Show Entries Values
            "aLengthMenu": [5, 10, 25, 50, 100, 250, 500, 1000, 5000, 10000],
                "sDom": 'T C<"clearfix">lfrtip',
            ...
            if ($(this).closest('.search_manually').length > 0) {
                ...
                // Create a unique ID for the search button based on the table index
                var searchBtnID = 'searchBtn3_' + index;
            ...
            OrdersTable.on('xhr', function () {
                ...
                if ('function' == typeof window.orderPrice) {
                    orderPrice();
                } else if ('function' == typeof window.reloadTable) {
                    reloadTable();
                } else {
                    // do something else
        ...
        $('.dTable_ajaxOrder').each(function (index) {
            // Initialize variables and prepare the URL
            var url4 = $(this).attr("data-href"); // Get URL from data attribute
            var uniq4 = $(this).attr("data-uniq");
            var cols4 = ('undefined' === typeof $(this).attr("data-cols")) ? false : $(this).attr("data-cols");
            var sorting4 = ('undefined' === typeof $(this).attr("data-sorting")) ? false : $(this).attr("data-sorting");

            // Add uniq to the URL
            url4 = url4 + "&uniq=" + uniq4;
            // console.log("Initial URL: " + url4); // Debugging the URL at initialization

            // Define columnDefs for both six and seven columns
            var columnDefsSixColumns4 = [
                { "targets": [0], "orderable": false },
                { "targets": [1], "orderable": false },
                { "targets": [2], "orderable": false },
                { "targets": [3], "orderable": false },
            ...
            var columnDefsSevenColumns4 = [
                { "targets": [0], "orderable": true },
                { "targets": [1], "orderable": true },
                { "targets": [2], "orderable": true },
                { "targets": [3], "orderable": true },
                { "targets": [4], "visible": false, "orderable": true },
                { "targets": [5], "visible": false, "orderable": true },
                { "targets": [6], "visible": true, "orderable": true },
                { "targets": [10], "visible": false, "orderable": true },
                { "targets": [-1], "orderable": false, className: 'tableBgGray custom_width' }
            ];

            // Determine columnDefs based on the number of columns
            var columnDefs4 = (cols4 == 6) ? columnDefsSixColumn4s : columnDefsSevenColumns4;
            var bSortValue4 = sorting4 == false ? false : true;

            // Initialize DataTable
            var OrdersTable4 = $(this).DataTable({
                // Show Entries Values        lues
                "lengthMenu": [5, 10, 25, 50, 100, 250, 500, 1000, 5000],
                "sDom": 'T C<"clearfix">lfrtip',
                "bSort": bSortValue4, // this will disable sorting all around, "orderable" inside "columnDefs" won't work 
                ...
                "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                    if (aData[14] == "1") {
                        $('td', nRow).css('background-color', 'rgba(255, 0, 0, 0.45)');
                    }
                },
                // Show AJAX Loading text or image
                oLanguage: {
                ...
                ajax: {
                    url: url4, // Use the dynamically constructed URL here
                    type: 'POST',
                    data: function (d) {
                        d.dateCodeFrom = dateCodeFrom;
                        d.dateCodeTo = dateCodeTo;
            ...
            if ($(this).closest('.search_manually').length > 0) {
                // Disable auto search on typing in the search input
                var dataTableSearchBox4 = $(this).closest('.dataTables_wrapper').find('.dataTables_filter input');
                dataTableSearchBox4.off();

                // Create a unique ID for the search button based on the table index
                var searchBtnID4 = 'searchBtn4_' + index;

                // Remove any existing search buttons to avoid duplication
                $(this).closest('.dataTables_wrapper').find('#' + searchBtnID4).remove();

                // Insert the search button dynamically after the search input with a unique ID
                $('<button id="' + searchBtnID4 + '" class="search_manually_btn form-control-feedback"><span class="glyphicon glyphicon-search"></span></button>').insertAfter(dataTableSearchBox4);

                // Trigger search manually on button click only for this specific table
                $('#' + searchBtnID4).on('click', function () {
                    var searchTerm4 = dataTableSearchBox4.val();
                    OrdersTable4.search(searchTerm4).draw(); // Trigger search only for the current table
                });

                // Trigger search when pressing 'Enter' key in the search input
                dataTableSearchBox4.on('keydown', function (e) {
                    if (e.key === 'Enter') { // Check if 'Enter' key is pressed
                        var searchTerm4 = dataTableSearchBox4.val();
                        OrdersTable4.search(searchTerm4).draw(); // Trigger search only for the current table
                    }
            ...
            // Process AJAX updates
            OrdersTable4.on('xhr', function () {
                function processOrders(url, colorClass) {
                    $.ajax({
                        type: "POST",
                        url: url,
                        success: function (res) {
                            ...
                            $tableRows.each(function () {
                                var rowId = $(this).find("td:eq(2)").text().trim();
                        ...
                        error: function (xhr, status, errorThrown) {
                            console.error(`Error fetching data from ${url}:`, xhr.responseText);
                        }
                    });
                }

                // Call to process the orders with their respective colors
                processOrders("order/order_ajax.php?page=readyOrdersCount", "lightblue");
                processOrders("order/order_ajax.php?page=semiReadyOrdersCount", "#e2c19c");
