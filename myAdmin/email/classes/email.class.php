...
class email extends object_class{
    ...
    public function newLetterAdd(){
        ...
        if(isset($_POST['submit'])){
            ...
            $order_show        = empty($_POST['order_show'])     ? "0"   : $_POST['order_show'];
            ...
            try{
                ...
                $sql = "INSERT INTO `email_letters` (`event`, `from_name`, `from_mail`, `reply_to`, `return_path`, `subject`, `order_show`, `message`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

                $array = [$event, $fromName, $fromMail, $replayTO, $replayTO, $subject, $order_show, $dsc];
    ...
    public function letterEditSubmit(){
        global $_e;
        if(isset($_POST['submit'])){
            if(!$this->functions->getFormToken('editNewsLetter')){
                return false;
            }

            $event = empty($_POST['event']) ? "" : serialize($_POST['event']);
            $fromName = empty($_POST['fromName']) ? "" : serialize($_POST['fromName']);
            $fromMail = empty($_POST['fromMail']) ? "" : $_POST['fromMail'];
            $subject = empty($_POST['subject']) ? "" : serialize($_POST['subject']);
            $dsc = empty($_POST['dsc']) ? "" : base64_encode(serialize($_POST['dsc']));
            $replayTO = empty($_POST['replayTO']) ? "" : serialize($_POST['replayTO']);
            $order_show = empty($_POST['order_show']) ? "0" : $_POST['order_show'];
            $lastId = $_POST['editId'];
            
            $languages = $this->functions->IbmsLanguages();
            if ($languages != false) {
                $lang_nonArray = implode(',', $languages);
            }
            
            foreach ($fromMail as $lang => $email) {
                $fromMail[$lang] = explode("@", $email)[0];
            }
            
            $fromMail = serialize($fromMail);

            try{
                $this->db->beginTransaction();

                $sql = "UPDATE `email_letters` SET `event` = ?, `from_name` = ?, `from_mail` = ?, `reply_to` = ?, `return_path` = ?, `subject` = ?, `order_show` = ?, `message` = ? WHERE id = ?";

                $array = [$event, $fromName, $fromMail, $replayTO, $replayTO, $subject, $order_show, $dsc, $lastId];
                $this->dbF->setRow($sql,$array,false);
    ...
    public function emailStats(){
        ...
        foreach($data as $val){
            ...
            $letterData =  $this->dbF->getRow($sql, [$val['letter_id']]);
    ...
    public function letterView(){
        ...
        echo '<div class="table-responsive">
                ...
                <tbody>';
        $sql  = "SELECT * FROM email_letters WHERE email_type = ? ORDER BY id DESC ";
        $data =  $this->dbF->getRows($sql, ['letter']);
        $i = 0;
    ...
    public function emailContentView(){
        ...
        echo '<div class="table-responsive">
                ...
                <tbody>';
        $sql  = "SELECT * FROM email_letters WHERE email_type != ? AND visible = ? ORDER BY id DESC";
        $data =  $this->dbF->getRows($sql, ['letter', 1]);
        $i = 0;
        $defaultLang = $this->functions->AdminDefaultLanguage();
        foreach($data as $val){
            $i++;
            $id = $val['id'];
            $event = unserialize($val['event']);
            $event = $event[$defaultLang];
            $subject = unserialize($val['subject']);
            $subject = $subject[$defaultLang];
            echo "<tr>
                    <td>$i</td>
                    <td>$event</td>
                    <td>$subject</td>
    ...
    public function letterNew()
    {
        ...
        if (isset($_GET['editId']) && $_GET['editId'] != '') {
            $editId = $_GET['editId'];
            $sql = "SELECT * FROM email_letters WHERE  id = ?";
            $data = $this->dbF->getRow($sql, [$editId]);
            $token = $this->functions->setFormToken('editNewsLetter', false);
            
            $eventValue = unserialize($data['event']);
            $fromNameValue = unserialize($data['from_name']);
            $fromMailValue = unserialize($data['from_mail']);
            $replyToValue = unserialize($data['reply_to']);
            $subjectValue = unserialize($data['subject']);
            $descValue = unserialize(base64_decode($data['message']));
        ...
        for ($i = 0; $i < sizeof($lang); $i++) {
            ...
            echo '<div class="form-group">
                                ...
                                <div class="col-sm-10  col-md-9">
                                    <input type="text" value="' . $eventValue[$lang[$i]] . '" name="event[' . $lang[$i] . ']"  maxlength="150"  class="form-control" placeholder="' . _u($_e['LETTER TITLE']) . '">
                                </div>
                        </div>';
            ...
            echo '<div class="form-group">
                            ...
                            <div class="col-sm-10  col-md-9">
                                    <input type="text" value="' . $fromNameValue[$lang[$i]] . '"  name="fromName[' . $lang[$i] . ']"  maxlength="150"  class="form-control" placeholder="' . _u($_e['FROM NAME']) . '">
                            </div>
                        </div>';
            ...
            echo '<div class="form-group">
                            ...
                            <div class="col-sm-10  col-md-9">
                                <input type="email" value="' . $fromMailValue[$lang[$i]] . '@' . $this->functions->defaultEmail . '"  name="fromMail[' . $lang[$i] . ']"  maxlength="150"  class="form-control" placeholder="' . _u($_e['FROM MAIL']) . '">
                            </div>
                        </div>';
            ...
            if (empty($data['email_type']) || @$data['email_type'] == "letter") {
                ...
                echo '<div class="form-group">
                                ...
                                <div class="col-sm-10  col-md-9">
                                    <input type="email" value="' . $replyToValue[$lang[$i]] . '"  name="replayTO[' . $lang[$i] . ']"  maxlength="150"  class="form-control" placeholder="' . _u($_e['REPLAY TO']) . '">
                                </div>
                            </div>';
            }
            //SUBJECT
            echo '<div class="form-group">
                            ...
                            <div class="col-sm-10  col-md-9">
                                <input type="text" value="' . $subjectValue[$lang[$i]] . '"  name="subject[' . $lang[$i] . ']"  maxlength="150"  class="form-control" placeholder="' . _u($_e['SUBJECT']) . '">
                            </div>
                        </div>';
            ...
            echo '<div class="form-group">
                            ...
                            <div class="col-sm-10  col-md-9">
                                <textarea name="dsc[' . $lang[$i] . ']" placeholder="' . _u($_e['Enter Full Detail']) . '" class="ckeditor">' . $descValue[$lang[$i]] . '</textarea>
                                <br>
        ...
        // Order Show
        $checked = "";
        if (@$data['order_show'] == '1') {
            $checked = 'checked';
        }
        echo '<div class="form-group">
                        <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Show On Order Page']) . '</label>
                        <div class="col-sm-10  col-md-9">
                            <div class="make-switch" data-off="danger" data-on="success" data-on-label="' . _uc($_e['Yes']) . '" data-off-label="' . _uc($_e['No']) . '">
                                <input type="checkbox" name="order_show" value="1" ' . $checked . '>
                            </div>
                        </div>
                    </div>';
