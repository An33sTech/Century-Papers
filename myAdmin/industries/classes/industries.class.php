...
class industries extends object_class
{
    public $productF;
    public function __construct()
    {
        ...
        $_w = array();
        //index page
        $_w['Industries Management'] = '';
        //industries.php
        $_w['Manage Industries'] = '';
        $_w['Active Industries'] = '';
        $_w['Pending Industries'] = '';
        $_w['Draft Industries'] = '';
        $_w['Add New Industries'] = '';
        ...
        $_w['ACTION'] = '';
        $_w['Industries'] = '';
        $_w['Industries Save Successfully'] = '';
        $_w['Added'] = '';
        $_w['Industries Save Failed'] = '';
        $_w['Industries Update Successfully'] = '';
        $_w['Industries Update Failed'] = ''; // <-- added
        ...
        $_w['Industries Image'] = '';
        $_w['Industries Icon'] = '';
        $_w['Old Industries Image'] = '';
        $_w['Old Industries Icon'] = '';
        $_w['SAVE'] = '';
        $_w['Slug'] = '';
        $_w['SLUG'] = '';
        $_w['Industries Middle Image'] = '';
        $_w['Old Industries Middle Image'] = '';
        $_w['Industries Top Image'] = '';
        $_w['Old Industries Top Image'] = '';
        $_w['Industries Main Image'] = '';
        $_w['Industries Banner Video'] = '';
        $_w['Old Industries Banner Video'] = '';
    ...
    public function serviceView()
    {
        ...
        echo '<div class="table-responsive">
                ...
                <tbody>';
        $sql = "SELECT * FROM `industries` WHERE `publish` = '1'";
        ...
        foreach ($data as $val) {
            ...
            $slug = $this->db->industryPage . $val['slug'];
            $seoLink = '';
            if ($this->functions->developer_setting('seo') == '1') {
                $this->functions->getAdminFile("seo/classes/seo.class.php");
                $seoC = new seo();
                $seoLink = $seoC->seoQuickLink($id, urlencode("/" . $this->db->industryPage . "$val[slug]"));
            }
            echo "<tr>
                    <td>$i</td>
                    <td>{$heading}</td>
                    ...
                    <td>
                        <div class='btn-group btn-group-sm'>
                            $seoLink
                            <a data-id='$id' href='-industries?page=edit&industriesId=$id' class='btn'>
    ...
    public function servicePending()
    {
        ...
        $sql = "SELECT `id`, `slug`, `heading`, `dateTime` FROM `industries` WHERE `publish` = '1'";
        $data = $this->dbF->getRows($sql);
        $i = 0;
    ...
    public function serviceDraft()
    {
        ...
        echo '<div class="table-responsive">
                ...
                <tbody>';
        $sql = "SELECT `id`, `slug`, `heading`, `dateTime` FROM `industries` WHERE `publish` = '0'";
        $data = $this->dbF->getRows($sql);
        $i = 0;
    ...
    public function newServiceAdd()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $date        = empty($_POST['date'])        ? '' : serialize($_POST['date']);
            ...
            try {
                ...
                if ($file) {
                    $imgName = $this->functions->uploadSingleImage($_FILES['image'], 'industries/');
                    if ($imgName == false) {
                ...
                if ($icon) {
                    $imgName2 = $this->functions->uploadSingleImage($_FILES['icon'], 'industriesicon/');
                    if ($imgName2 == false) {
                        throw new Exception(_uc($_e["Image File Error"]));
                    }
                }
                ...
                if ($image2) {
                    $imgName3 = $this->functions->uploadSingleImage($_FILES['image2'], 'industriesimage/');
                    if ($imgName3 == false) {
                        throw new Exception(_uc($_e["Image File Error"]));
                    }
                }
                ...
                if ($video) {
                    $imgName4 = $this->functions->uploadSingleImage($_FILES['video'], 'industriesvideo/');
                    if ($imgName4 == false) {
                        throw new Exception(_uc($_e["Image File Error"]));
                    }
                }

                $sql = "INSERT INTO `industries` (`heading`, `shortDesc`, `dsc`, `date`, `image`, `image2`, `video`, `icon`, `publish`) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

                $array = array($heading, $short_desc, $dsc, $date, $imgName, $imgName3, $imgName4, $imgName2, $publish);
                $this->dbF->setRow($sql, $array, false);
                $lastId = $this->dbF->rowLastId;

                // slug handling
                $sql_slug = "SELECT * FROM `industries` WHERE `slug` = ?";
                ...
                if ($slug == "") {
                    $slug = $this->db->industryPage . $lastId;
                }

                $sql = "UPDATE `industries` SET `slug` = ? WHERE `id` = ?";
                $this->dbF->setRow($sql, array($slug, $lastId), false);

                // SEO entry
                $pageLink = '/' . $this->db->industryPage . $slug;
                $ref_id   = $this->db->industryPage . $lastId;
                ...
                if ($this->dbF->rowCount > 0) {
                    $this->functions->notificationError(_uc($_e['Industries']), ($_e['Industries Save Successfully']), 'btn-success');
                    $this->functions->setlog(_uc($_e['Added']), _uc($_e['Industries']), $this->dbF->rowLastId, ($_e['Industries Save Successfully']));
                } else {
                    $this->functions->notificationError(_uc($_e['Industries']), ($_e['Industries Save Failed']), 'btn-danger');
                }
            } catch (Exception $e) {
                if ($imgName && $file) {
    ...
    public function serviceEditSubmit()
    {
        ...
        if (isset($_POST['submit']) && isset($_POST['editId'])) {
            ...
            try {
                ...
                if ($this->dbF->rowCount > 0) {
                    $this->functions->notificationError(_uc($_e['Industries']), ($_e['Industries Update Successfully']), 'btn-success');
                    $this->functions->setlog(_uc($_e['Added']), _uc($_e['Industries']), $this->dbF->rowLastId, ($_e['Industries Update Successfully']));
                } else {
                    $this->functions->notificationError(_uc($_e['Industries']), ($_e['Industries Save Failed']), 'btn-danger');
                }
            } catch (Exception $e) {
                ...
                $this->db->rollBack();
                $this->dbF->error_submit($e);
                $this->functions->notificationError(_uc($_e['Industries']), ($_e['Industries Update Failed']), 'btn-danger');
            }
        }
    }
    ...
    public function serviceNew()
    {
        ...
        if ($lang != false) {
            ...
            for ($i = 0; $i < $lang_size; $i++) {
                ...
                echo '<div class="form-group">
                            <label class="col-sm-2 col-md-3  control-label">' . _uc($_e['Industries Title']) . '</label>
                ...
                echo '<div class="form-group">
                            ...
                            <div class="col-sm-10  col-md-9">
                                <input type="date" name="date[' . $lang[$i] . ']" class="form-control" placeholder="Date">
                            </div>
                      </div>';   
        ...
        // echo '<div class="form-group">
        //         <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Industries Top Image']) . '</label>
        //         <div class="col-sm-10  col-md-9">
        ...
        // echo '<div class="form-group">
        //         <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Industries Middle Image']) . '</label>
        //         <div class="col-sm-10  col-md-9">
        ...
        // echo '<div class="form-group">
        //         <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Industries Banner Video']) . '</label>
        //         <div class="col-sm-10  col-md-9">
        ...
        if ($this->functions->developer_setting('blog_image') == '1') {
            echo '<div class="form-group">
                    <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Industries Main Image']) . '</label>
                    <div class="col-sm-10  col-md-9">
    ...
    public function serviceEdit()
    {
        ...
        $id    = $_GET['industriesId'];
        $sql   = "SELECT * FROM `industries` WHERE `id` = ?";
        $data  = $this->dbF->getRow($sql, array($id));

        echo '<form method="post" action="-industries?page=industries" class="form-horizontal" role="form" enctype="multipart/form-data">' .
            $token .
        ...
        if ($lang != false) {
            ...
            for ($i = 0; $i < $lang_size; $i++) {
                ...
                echo '<div class="form-group">
                            <label class="col-sm-2 col-md-3  control-label">' . _uc($_e['Industries Title']) . '</label>
                            <div class="col-sm-10  col-md-9">
                                <input type="text" value="' . htmlspecialchars($heading[$lang[$i]]) . '" name="heading[' . $lang[$i] . ']" class="form-control" placeholder="' . _uc($_e['Industries Title']) . '">
                            </div>
                        </div>';
                ...
                echo '<div class="form-group">
                            <label class="col-sm-2 col-md-3  control-label">Date</label>
                            <div class="col-sm-10  col-md-9">
                                <input type="date" value="' . htmlspecialchars($date[$lang[$i]]) . '" name="date[' . $lang[$i] . ']" class="form-control" placeholder="Date">
                            </div>
                        </div>';   
        ...
        //     echo '<div class="form-group">
        //         <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Old Industries Top Image']) . '</label>
        //         <div class="col-sm-10  col-md-9">
        ...
        //     echo '<div class="form-group">
        //         <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Old Industries Middle Image']) . '</label>
        //         <div class="col-sm-10  col-md-9">
        ...
        //     echo '<div class="form-group">
        //         <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Old Industries Banner Video']) . '</label>
        //         <div class="col-sm-10  col-md-9">
        ...
        if ($this->functions->developer_setting('blog_image') == '1') {
            if ($data['image'] != '') {
                ...
                echo '<div class="form-group">
                    <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Old Industries Image']) . '</label>
                    <div class="col-sm-10  col-md-9">
            ...
            echo '<div class="form-group">
                    <label  class="col-sm-2 col-md-3  control-label">' . _uc($_e['Industries Image']) . '</label>
                    <div class="col-sm-10  col-md-9">
                        <input type="file" name="image" class="btn-file btn btn-primary">
                    </div>
               </div>';
