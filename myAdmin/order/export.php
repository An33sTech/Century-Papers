...
$min = $_POST['min'];
$max = $_POST['max'];
$min = $date_from = (isset($_POST['min']) && $_POST['min'] != '' )  ? $_POST['min'] . ' 00:00:00'  : date('Y-m-d') . ' 00:00:00';
$max = $date_to   = (isset($_POST['max']) && $_POST['max'] != '' )    ? $_POST['max'] . ' 23:59:59'    : date('Y-m-d') . ' 23:59:59';
...
$sql = "SELECT ac.acc_id,ac.acc_name,ac.acc_email,order_invoice.* FROM `order_invoice`
                        ...
                        WHERE `invoice_date` >= '$min' AND `invoice_date` <= '$max' ORDER BY order_invoice_pk DESC";
