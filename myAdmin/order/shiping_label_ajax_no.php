...
$soldTo = [
    "Name" => "ELSpecialisten X3MSELLER",
    "AttentionName" => "ELSpecialisten X3MSELLER",
    "Address" => [
        "AddressLine" => [
            "Glimmerveien 1"
        ],
...
foreach ($productArr as $key => $val) {
    ...
    $proname = (strlen($val['desc']) > 35) ? substr($val['desc'], -35) : $val['desc'];
    $products[] = [
        "Description" => "$proname",
        "Unit" => [
            "Number" => "$val[qty]",
...
if ($error) {
    ...
    if ($error) {
        ...
        if (isset($res["ShipmentResponse"]["ShipmentResults"]["PackageResults"]["ShippingLabel"]["GraphicImage"])) {
            ...
            $data = $dbF->setRow($sql, [$invoi_idd, $trackId, $labelcode, $invoiveLabelCode]);
