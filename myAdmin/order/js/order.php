...
if (1 === 2) {
    ...
    getOrderProductJson = function (country, ths) {
        ...
        $.ajax({
            ...
            if (data == '[]') {
                $('#loadingProgress').html(
                    '<?php echo _js(_replace('{{option}}', '\'+$(ths).find("option:selected").text()+\'', $_e["NO Product Found In {{option}}"])); ?>'
                );
                $('#productNotFoundInCountry').addClass('orderAddProductNotAllow');
                jsonData(data);
            } else {
    ...
    scale = function (data) {
        ...
        $(scaleId).autocomplete({
            ...
            select: function (event, ui) {
                ...
                $(scaleHiddenClass).val(ui.item.label).attr("data-val", ui.item.id);
    ...
    color = function (data) {
        ...
        $(colorId).autocomplete({
            ...
            return $("<li></li>")
                ...
                .append("<div style='background:#" + item.label + ";color:#FFF;height:100%;'>" + item.label +
                    "</div>")
                .appendTo(ul);
        };
    };
    ...
    productStoreJson = function (pid, scaleid, colorid, country) {
        ...
        $.ajax({
            ...
            if (data == '[]') {
                ...
                notification('<?php echo _js($_e['Product Inventory Error']); ?>',
                    '<?php echo _js($_e['Store not found please try other product. OR Product has no stock avaiable.']); ?>',
                    "btn-danger");
                jsonDataStore(data);
            } else {
    ...
    qtyCheck = function () {
        ...
        if (val <= valReal && val > 0) {
            ...
            notification('<?php echo _js($_e['Quantity Error']); ?>',
                '<?php echo _js($_e['Your Selected Quantity is Greater then or less then real quantity Please Check. Use Mouse Scroll or UP/DOWN arrow to select quantity']); ?>',
                'btn-warning');
            return false;
        }
    };
    ...
    tempTotalPrice = function () {
        ...
        if (total < 0) {
            ...
            notification('<?php echo _js($_e["Price Error"]); ?>',
                '<?php echo _js($_e["Your Total Price Of Product Is Not OK, Please Check."]); ?>', "btn-warning");
            return false;
        }
        return total;
    };
    ...
    tempAddProductValid = function () {
        ...
        if (store == "") {
            notification('<?php echo _js($_e["Product Error"]); ?>',
                '<?php echo _js($_e["Before Continue Please Select Product Or Store."]); ?>', "btn-danger");
            return false;
        }
    ...
    function invoiceFormValid() {
        ...
        if ($(".invoice_product_id").val() == "" || $("#invoice_product_store").val() == "") {
            ...
            notification('<?php echo _js($_e["Required Fields Are Empty"]); ?>',
                '<?php echo _js($_e["Please enter proper data in all fields."]); ?>', "btn-warning");
            return false;
        }
        addListItem();
    ...
    function addListItem() {
        ...
        var pScaleId = parseInt($(".invoice_product_scale.has").data('val'));
        ...
        var pName = $(".invoice_product_id").attr('data-val') + " -- " + $(".invoice_product_scale").attr('data-val') +
            " -- " + $(".invoice_product_color").attr('data-val');
        ...
        if (document.getElementById("tr_" + trpid)) {
            notification('<?php echo _js($_e["Duplicate Entry"]); ?>',
                '<?php echo _js($_e["Duplicate Entry : Product Item already exist in list!"]); ?>', 'btn-danger');
            ...
            var item = "<tr id='tr_" + trpid + "' data-id='" + trpid + "'>" +
                "<td><input type='checkbox' id='" + trpid + "' onchange='checkchange(this.id)' value='" + trpid +
                "' class='checkboxclass' />" +
                "<span>" + sr + "</span></td>" +
                "<td>" + pName +
                ...
                "<input type='hidden' name='pTotalprice_" + trpid + "' value='" + totalPrice +
                "' class='addedTotalPrice'/></td>" +
                "<td>" + pWeight + " KG</td>" +
                "<td>" + qty + "</td>" +
    ...
    function isInternationalShipping() {
        ...
        if (receiverCountry == "") {
            notification('<?php echo _js($_e["Required Fields"]); ?>',
                '<?php echo _js($_e["Please Enter Sender / Required Fields."]); ?>', "btn-warning");
            return false;
        }
        ...
        if (!chk) {
            notification('<?php echo _js($_e["Shipping Error"]); ?>',
                '<?php echo _js($_e["Select product Are not ship in receiver country."]); ?>', "btn-danger");
            return false;
        }
        return chk;
    }
    ...
    function formSubmit() {
        ...
        if ($('#vendorProdcutList tr').length <= 0) {
            notification('<?php echo _js($_e["Product Error"]); ?>',
                '<?php echo _js($_e["You not select any product for order."]); ?>', "btn-warning");
            return false;
        }
        ...
        if (payment == "") {
            notification('<?php echo _js(_uc($_e["Payment Info"])); ?>',
                '<?php echo _js($_e["Payment Type not select Befor continue please select payment type."]); ?>',
                "btn-warning");
            return false;
        }
    ...
    function finalFormSubmit() {
        //check on formsubmit is product selected
        if ($('#vendorProdcutList tr').length <= 0) {
            notification('<?php echo _js(_uc($_e["Product Error"])); ?>',
                '<?php echo _js($_e["You not select any product for order."]); ?>', "btn-warning");
            return false;
        }
        ...
        if (payment == "") {
            notification('<?php echo _js(_uc($_e["Payment Info"])); ?>',
                '<?php echo _js(_fc($_e["Payment Type not select Befor continue please select payment type."])); ?>',
                "btn-warning");
            return false;
        }
    ...
    flagOrderToTop = function (ths) {
        ...
        $.ajax({
            url: 'order/order_ajax.php?page=flagOrder&id=' + id,
            type: 'POST',
            data: {
                itemId: id
            }
    ...
    removeFlagOrderToTop = function (ths) {
        ...
        $.ajax({
            url: 'order/order_ajax.php?page=removeFlagOrder&id=' + id,
            type: 'POST',
            data: {
                itemId: id
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (shippingData.find == '1') {
                        ...
                        if (shp_int == '0') {
                            jAlertifyAlert(
                                '<?php echo _js(_replace('{{country}}', "'+shp_int+'", $_e['Its Look Like Shipping Stop In Receiver Country {{country}}'])); ?>'
                            );
                            return false;
                        }
                        $('.topViewP').fadeIn(500);
                        ...
                        jAlertifyAlert(
                            '<?php echo _js($_e['Its Look Like Shipping Is Not Available In Receiver Country.']); ?>',
                            'Shipping Error');
    ...
    function quick_invoice_update(orderid, ths) {
        ...
        if (selected_id == "1" || selected_id == "3" || selected_id == "0" || selected_id == "6") {


            $(".deleteStatus").show(200);


        } else {

            $(".deleteStatus").hide(200);
        ...
        if (secure_delete("Do you want to update???")) {
            ...
            $('input[name="dStock[]"]:checked').each(function () {
                dStock.push(this.value);
            });
            ...
            $.ajax({
                ...
                data: {
                    orderid: orderid,
                    invoice: selected_id,
                    prev_status: pre_status,
                    dStock: dStock
                }
            }).done(function (data) {

                if (data['val'] !== undefined && data['val'].includes("Error :")) {
                    jAlertifyAlert(data['val']);
                }
                data = JSON.parse(data)
                if (data['id'] == '1') {

                    $(ths).closest("td").find(".invoice_quick_select_div").hide(200);
