...
class webUsers extends object_class{
    ...
    public function webUsersView(){
        ...
        foreach($data as $val){
            ...
            if($product){
                ...
                $totalDone      =    $productClass->totalOrderSubmit($id,'3');
                $totalPending   =    $productClass->totalOrderSubmit($id,'2');
                $totalCancel    =    $productClass->totalOrderSubmit($id,'0');
                $total_other    =    $productClass->totalOrderSubmit($id,'NOT IN (0,2,3)');

                echo "<td>$totalSubmit</td>";
                echo "<td>
                        <div class='btn-group input-group'>
                            <div class='btn btn-success'    title='"._u($_e['ORDER DONE'])."'>$totalDone</div>
                            <div class='btn btn-info'       title='"._u($_e['ORDER PENDING'])."'>$totalPending</div>
    ...
    public function webUsersPending(){
        ...
        echo '<div class="table-responsive">
                ...
                <tbody>';
        $sql  = "SELECT * FROM accounts_user WHERE acc_type = '0' ORDER BY acc_id DESC limit 1500";
        $data =  $this->dbF->getRows($sql);
        $i = 0;
        foreach($data as $val){
            $i++;
            ...
            try {
                ...
                if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $sql = "UPDATE  accounts_user SET
                                ...
                                acc_type = ?
                                WHERE acc_id = ?";
                    $array = array($name,$email,$type,$id);
                    $this->dbF->setRow($sql,$array,false);
            ...
            try {
                ...
                if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $sql = "UPDATE  accounts SET
                                ...
                                acc_role  =?
                                WHERE acc_id = ?";
                    $array = array($name,$email,$type,$role,$id);
                    $this->dbF->setRow($sql,$array,false);
    ...
    public function adminGrpEditSubmit(){
        ...
        if (isset($_POST['name']) && !empty($_POST['name'])
            ...
            $sql    =   "UPDATE `accounts_prm_grp` SET
                            `name` = ?,
                            `permission` = ? WHERE id = ?";
            $this->dbF->setRow($sql,array($name,$makeNewLink,$id));
            //var_dump($makeNewLink);
