...
class product_function extends object_class
{
    ...
    function __construct()
    {
        ...
        $_w['Order will be sent from factory by DHL EXPRESS'] = '';
        $_w['PRIORITY 1 URGENT DELIVERY'] = '';
        $_w['ORDERED TO MAIN STOCK'] = '';
        $_w['MADE TO MEASURE ORDER'] = '';
        $_w['Manually Created'] = '';
    ...
    {
        $sql = "UPDATE proudct_detail SET `sale` = `sale` + ? WHERE prodet_id = ?";
        $this->dbF->setRow($sql,array($qty, $pId));
    }

    public function productDealCountPlus($dealId, $qty)
    {
        $sql = "UPDATE product_deal SET `sale` = `sale` + ? WHERE id = ?";
        $this->dbF->setRow($sql,array($qty, $dealId));
    }
    ...
    {
        $qry = "SELECT * FROM  `product_setting` WHERE `p_id` = ?";
        $eData = $this->dbF->getRows($qry, array($pId));
        if ($this->dbF->rowCount > 0) {
            return $eData;
        } else {
            return false;
    ...
    public function productQTY($pid, $storeId = '0', $scaleId = '0', $colorId = '0', $exactValue = false)
    {
        ...
        if ($storeId == '0' && $exactValue == false)
            $storeId = " >= '$storeId'";
        else
            $storeId = " = '$storeId'";

        if ($scaleId == '0' && $exactValue == false)
            $scaleId = " >= '$scaleId'";
        else
            $scaleId = " = '$scaleId'";

        if ($colorId == '0' && $exactValue == false)
            $colorId = " >= '$colorId'";
        else
            $colorId = " = '$colorId'";
    ...
    public function productPrice($productId, $currId = '')
    {
        ...
        elseif($currId == '26'){
            $sql = "SELECT * FROM `product_price` WHERE  `propri_prodet_id` = '$productId' AND `propri_cur_id` = 20";
            $data = $this->dbF->getRow($sql);
            $data['propri_price'] = ceil(($data['propri_price'] / 11) * 1.1);
            $data['propri_cur_id'] = 26;
            return $data;
        }else{
            return 0;
        }
    ...
    public function scaleInventory($pId, $storeId, $colorGreaterThenZero = true)
    {
        ...
        if ($colorGreaterThenZero == true) {
            ...
            $colorGreaterThenZero = "AND (qty_product_color = '0' OR qty_product_color IS NULL)";
        }
    ...
    public function hasInventoryInScale($inventory, $pId, $scaleName)
    {
        foreach($inventory as $key => $val){
            if($val['qty_product_id'] == $pId && $val['qty_product_scale_name'] == $scaleName && intval($val['qty_item']) > 0){
                return $key;
            }
        }
        // foreach ($inventory as $key => $val) {
    ...
    public function scaleWithInventory($pId, $currency, $storeId, $hasColor = false)
    {
        ...
        foreach ($scaleData as $key => $val) {
            $productId = $val['prosiz_prodet_id'];
            $sizeName = $val['prosiz_name'];
            $scaleId = $val['prosiz_id'];
            $name = $val['prosiz_name'];
            $hasInventoryInScale = $this->hasInventoryInScale($scaleInventory, $productId, $sizeName);
            if (in_array($name, $scales_names))
                continue;
     ...
     * @return bool
     */
    function stockProductQtyMinus($hash, $qty, $order_invoice_id = '0')
    {
        global $dbF;

        $sql = "SELECT * FROM `product_inventory` WHERE `product_store_hash` = ?";
        $inventoryData = $this->dbF->getRow($sql, [$hash]);
        ...
        $pName = $this->getProductName($qty_product_id);
        $sName = $this->getScaleName($qty_product_scale);

        if ($invQty > 0 && $invQty >= $qty) {
            $sql = "UPDATE `product_inventory` SET `qty_item` = `qty_item` - ? WHERE `product_store_hash` = ?";
            $this->dbF->setRow($sql,array($qty, $hash));

            $pMmsg = $qty . " qty REMOVE successfully " . $pName . " " . $sName;
            echo "<div class='alert alert-danger'>$pMmsg</div>";
            $publish_date = date('Y-m-d h:i:a');

            $sqlg = "INSERT INTO `invLogs` (`type`, `hash`, `sid`, `pid`, `minusQty`, `addQty`, `publish_date`, `heading`, `oid`) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ";
            $this->dbF->setRow($sqlg, ['order completed', $hash, $qty_product_scale, $qty_product_id, $qty, '', $publish_date, 'sharkspeed.com', $order_invoice_id]);
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $this->dbF->setRow($sqlg_, ['Order Completed', 'Sharkspeed MyAdmin', $hash, $ref_id, $user_email, '', $qty, $qty_product_id, $qty_product_scale, $order_invoice_id, $desc, $ip, $browser]);

            $sqlc = "INSERT INTO `removeQTY` (`qty`,`hash`,`order`) VALUES (?,?,?)";
            $array = array($qty, $hash, $order_invoice_id);
            $this->dbF->setRow($sqlc, $array);

            return $qty;
        } elseif ($qty > $invQty && $invQty != 0) {
            $sql = "UPDATE `product_inventory` SET `qty_item` = `qty_item` - ? WHERE `product_store_hash` = ?";
            $this->dbF->setRow($sql,array($invQty, $hash));

            $pMmsg = $invQty . " qty REMOVE successfully " . $pName . " " . $sName;
            ...
            $sqlg = "INSERT INTO `invLogs`(`type`, `hash`, `sid`, `pid`, `minusQty`, `addQty`, `publish_date`, `heading`, `oid`) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ";
            $this->dbF->setRow($sqlg, ['Order Completed', $hash, $qty_product_scale, $qty_product_id, $invQty, '', $publish_date, 'sharkspeed.com', $order_invoice_id]);

            $sqlc = "INSERT INTO `removeQTY` (`qty`,`hash`,`order`) VALUES (?,?,?)";
            $array = array($invQty, $hash, $order_invoice_id);
            $this->dbF->setRow($sqlc, $array);
     ...
     * @return bool
     */
    public function stockProductQtyMinusIntra($hash, $qty)
    {
        global $conIntra;

        $sqli = "SELECT `qty_item` FROM `product_inventory` WHERE `product_store_hash` = '$hash'";
        $inventoryData = $conIntra->getRow($sqli);
        $invQty = $inventoryData[0];
    ...
    public function stockProductQtyMinusIntraNEW($hash, $qty, $order_invoice_id)
    {
        global $conIntra;
        ...
        if ($conIntra->rowCount <= 0) {
            ...
            $sql = "SELECT * FROM `product_inventory` WHERE `product_store_hash` = '$hash'";
            $dataz = $this->dbF->getRow($sql);
            
            if ($this->dbF->rowCount > 0) {
                $sqlz = "INSERT INTO `product_inventory`(`qty_store_id`, `qty_product_id`, `qty_product_scale`, `qty_product_color`, 
                `qty_item`, `location`, `product_store_hash`) VALUES (?,?,?,?,?,?,?)";

                $invQty1 = $dataz['qty_item'];

                if ($invQty1 > 0 && $invQty1 >= $qty) {



                    $array = array($dataz['qty_store_id'], $dataz['qty_product_id'], $dataz['qty_product_scale'], $dataz['qty_product_color'], $dataz['qty_item'] - $qty, $dataz['location'], $dataz['product_store_hash']);


                } elseif ($qty > $invQty1 && $invQty1 != 0) {



                    $array = array($dataz['qty_store_id'], $dataz['qty_product_id'], $dataz['qty_product_scale'], $dataz['qty_product_color'], $dataz['qty_item'] - $invQty1, $dataz['location'], $dataz['product_store_hash']);


                }


                $conIntra->setRow($sqlz, $array);
            }

            $invQty = $dataz['qty_item'];
        } else {
            $invQty = $inventoryData[0];
        }
        if ($invQty > 0 && $invQty >= $qty) {
            ...
            $sqlc = "INSERT INTO `removeQTY` (`qty`,`hash`,`order`) VALUES (?,?,?)";
            $array = array($qty, $hash, $order_invoice_id);
            $conIntra->setRow($sqlc, $array);

            return $qty;

        } elseif ($qty > $invQty && $invQty != 0) {
           
            $sql = "UPDATE `product_inventory` SET `qty_item` = qty_item-'$invQty' WHERE `product_store_hash` = '$hash'";
            $conIntra->setRow($sql);
     ...
     * @todo used in product return in returns.php
     */
    public function stockProductQtyPlusIntra($hash, $qty)
    {
        global $conIntra;
        $sql = "UPDATE `product_inventory` SET `qty_item` = qty_item+'$qty' WHERE `product_store_hash` = '$hash'";
        $conIntra->setRow($sql);
     ...
     * @todo used in product return in returns.php
     */
    public function stockProductQtyPlusIntraNEW($hash, $qty, $order_invoice_id)
    {
        global $conIntra;



        $sqli = "SELECT `qty` FROM `removeQTY` WHERE `hash` = '$hash' AND `order` = '$order_invoice_id'";
        $inventoryData = $conIntra->getRow($sqli);

        if ($conIntra->rowCount > 0) {


            $qtyInve = $inventoryData[0];


            if ($qtyInve > 0 && $qtyInve >= $qty) {
                ...
                //  $sqlf    =   "UPDATE `removeQTY` SET `qty` = qty-'$qty' WHERE `hash` = '$hash'";
// $conIntra->setRow($sqlf);

                // if($conIntra->rowCount > 0)
//     return true;
                return $qty;
            } elseif ($qty > $qtyInve && $qtyInve != 0) {
                // echo "HELLOOOOOO333333";
                ...
                $sql = "UPDATE `product_inventory` SET `qty_item` = qty_item+'$qtyInve' WHERE `product_store_hash` = '$hash'";
                $conIntra->setRow($sql);



                //  $sqlf    =   "UPDATE `removeQTY` SET `qty` = qty-'$qtyInve' WHERE `hash` = '$hash'";
// $conIntra->setRow($sqlf);
    ...
    public function scaleSQL($id, $columnName = '*', $groupByName = true)
    {
        ...
        $newSQL = "SELECT sort FROM product_size WHERE prosiz_prodet_id = '$id' $groupByName";
        $sortData = $this->dbF->getRows($newSQL);
        $hasNullSort = false;
        ...
        if($hasNullSort){
            $sql = "SELECT " . $columnName . " FROM `product_size` WHERE `prosiz_prodet_id` = '$id' $groupByName ORDER BY CASE
                    WHEN `prosiz_name` = 'XXS' THEN 1
                    WHEN `prosiz_name` = 'XS' THEN 2
                    WHEN `prosiz_name` = 'S' THEN 3
                    WHEN `prosiz_name` = 'M' THEN 4
                    WHEN `prosiz_name` = 'L' THEN 5
                    WHEN `prosiz_name` = 'XL' THEN 6
                    WHEN `prosiz_name` = 'XXL' THEN 7
                    WHEN `prosiz_name` = '2XL' THEN 8
                    WHEN `prosiz_name` = '3XL' THEN 9
                    WHEN `prosiz_name` = '4XL' THEN 10
                    WHEN `prosiz_name` = '5XL' THEN 11
                    WHEN `prosiz_name` = '6XL' THEN 12
                    END ASC, sort ASC";
            return $this->dbF->getRows($sql);
        }else{
            $sql = "SELECT " . $columnName . " FROM `product_size` WHERE `prosiz_prodet_id` = '$id' $groupByName ORDER BY sort ASC";
            return $this->dbF->getRows($sql);
        }
    }
    ...
    public function invoiceStatusAdmin($i)
    {
        //Invoice status select box create
        $invoice = $this->invoiceStatusArray();
        $option = '';
        ...
        foreach ($invoice as $key => $val) {
            ...
            if ($i == $key) {
                $f = 'selected';
                $option .= "<option $f value='$key'>$val</option>";
            } else {
    ...
    //     $sql = "SELECT `id`, `event` FROM `email_letters` WHERE `id` BETWEEN ? AND ?";
    //     $res = $this->dbF->getRows($sql, array(252,257));

    //     $option = '';
    //     foreach($res as $key=>$val){
    //         $option.= "<option value='".$val['id']."'>".$val['event']."</option>";
    //     }
    //     return $option;
    // }
    ...
    public function getOrderEmailTemplate()
    {
        ...
        $sql = "SELECT `id`, `event` FROM `email_letters` WHERE `order_show` = ?";
        $res = $this->dbF->getRows($sql, array(1));
    ...
    public function invoiceStatusArray()
    {
        ...
        $arr = array(
            ...
            3 => _uc($_e['Complete']),
            1 => _uc($_e['Denied']),
            4 => _uc($_e['Order will be sent from factory by DHL EXPRESS']),
            8 => _uc($_e['PRIORITY 1 URGENT DELIVERY']),
            12 => _uc($_e['ORDERED TO MAIN STOCK']),
            13 => _uc($_e['MADE TO MEASURE ORDER']),
            99 => _uc($_e['Manually Created'])
    ...
    public function productSale($pId, $countryId, $discount)
    {
        ...
        if (!empty($data) && $data != false && $this->productSaleData != false) {
            ...
            foreach ($data as $key => $value) {
                if (intval($value['pSale_discount']) === 0) {
                ...
                if ($this->dbF->rowCount > 0) {
                    $return = array();
                    $return['discount'] = $value['pSale_price_price'];
...
where `product_discount_pk` in
(SELECT product_dis_id FROM product_discount_setting WHERE
product_dis_id in (SELECT product_dis_id FROM `product_discount_setting` WHERE `product_dis_name` ='dateFrom' AND `product_dis_value` <= '$today') AND
product_dis_id in (SELECT product_dis_id FROM `product_discount_setting` WHERE `product_dis_name` ='dateTo' AND (`product_dis_value` >= '$today' OR `product_dis_value` = '')))
AND product_dis_curr_Id = '$countryId' AND product_dis_intShipping = '1'
AND discount_PId = '$pId'";
        ...
        //                  WHERE product_dis_status = '1'
        //                          AND `product_discount_pk` in
        //                                (SELECT product_dis_id FROM product_discount_setting WHERE          product_dis_id in (SELECT product_dis_id FROM `product_discount_setting` WHERE `product_dis_name` ='dateFrom' AND `product_dis_value` <= '$today') AND
        //                                product_dis_id in (SELECT product_dis_id FROM `product_discount_setting` WHERE `product_dis_name` ='dateTo' AND (`product_dis_value` >= '$today' OR `product_dis_value` = '')))
        //                          AND `product_discount_prices`.product_dis_curr_Id = '$countryId' AND product_discount_prices.product_dis_intShipping = '1'
        //                          AND discount_PId = '$pId' ";
        $data = $this->dbF->getRow($sql);
    ...
    public function getCategoryNames($categories, $echo = true)
    {
        if ($categories != '') {
            ...
            $sql = "SELECT `name` FROM `categories` WHERE id in ($categories) ORDER BY `name` Asc";
            $data = $this->dbF->getRows($sql);
            $names = "";
            if ($this->dbF->rowCount > 0) {
                foreach ($data as $val) {
    ...
    public function productSpecialImage($id, $alt, $defaultFirstImageShowForMain = true)
    {
        ...
        if ($imag == '') {
            if ($defaultFirstImageShowForMain && strtolower($alt) == 'main') {
                $sql = "SELECT * FROM `product_image` WHERE product_id = '$id' ORDER BY sort ASC ";
                $data = $this->dbF->getRow($sql);
                $imag = isset($data['image']) ? $data['image'] : '';
            }
        }
        return $imag;
    }
    ...
    public function has_buy_get_free_offer($order_id, $product_id)
    {
        ...
        if (is_array($data) && isset($data["setting_val"])) {
    ...
    public function getSubCatIds($parent)
    {
        ...
        $sql = "SELECT * FROM `categories` WHERE id = '$parent' OR under = '$parent'";
        $data = $this->dbF->getRows($sql);
        $cat = array();
        if ($this->dbF->rowCount > 0) {
            //1 2 dept
    ...
    public function get_staple_category()
    {
        $staple_ibm_cat = intval($this->functions->ibms_setting("stapleProduct"));
        if ($staple_ibm_cat > 0) {
            $staple_ibm_cat = $this->getSubCatIdsNew($staple_ibm_cat);
            return $staple_ibm_cat;
        } else {
            return false;
        }
    }

    public function check_product_in_staple_cat($pId)
    {
        $staple_category = $this->get_staple_category();
        $pro_cat = $this->product_category($pId);

        if (sizeof(@array_intersect(@$staple_category, $pro_cat)) > 0) {
            return true;
        }
        return false;
    }
    ...
    public function getProductSingleImage($prod_id)
    {
        $sql = "SELECT `image` FROM `product_image` WHERE `product_id` = ? ORDER BY `sort` LIMIT 1";
        $res = $this->dbF->getRow($sql, array($prod_id));
        return $res;

    }

    public function getAvailableGiftCards($currency = false)
    {
        $sql = "SELECT * FROM `gift_card` WHERE `sale` = 0 AND `currency` = '$currency'";
        $res = $this->dbF->getRows($sql);

        $option = '';
        foreach ($res as $key => $val) {
            $name = translateFromSerialize($val['name']);
            $option .= "<option value='" . $val['giftId'] . "'>" . $name . "</option>";
        }
        return $option;
    }
    

    public function getOrderLanguage($invoiceId)
    {
        $langSQL = "SELECT `language` FROM `order_invoice` WHERE `order_invoice_pk` = '$invoiceId'";
        $dataLang = $this->dbF->getRow($langSQL);

        if ($this->dbF->rowCount > 0) {
            return $dataLang["language"];
        } 

        $langSQL2 = "SELECT `language` FROM `order_invoice` WHERE `invoice_id` = '$invoiceId'";
        $dataLang2 = $this->dbF->getRow($langSQL2);

        if ($this->dbF->rowCount > 0) {
            return $dataLang2["language"];
        }
        
        return "Swedish";
    }    
    
    public function getOrderCurrencyId($invoiceId)
    {
        $codeSQL = "SELECT `price_code` FROM `order_invoice` WHERE `order_invoice_pk` = '$invoiceId'";
        $dataCode = $this->dbF->getRow($codeSQL);

        if ($this->dbF->rowCount > 0) {
            $curSQL = "SELECT `cur_id` FROM `currency` WHERE `cur_name` = '$dataCode[price_code]' OR `cur_symbol` = '$dataCode[price_code]'";
            $dataCur = $this->dbF->getRow($curSQL);
            
            if ($this->dbF->rowCount > 0) {
                return $dataCur["cur_id"];
            }
        } 

        $codeSQL2 = "SELECT `price_code` FROM `order_invoice` WHERE `invoice_id` = '$invoiceId'";
        $dataCode2 = $this->dbF->getRow($codeSQL2);

        if ($this->dbF->rowCount > 0) {
            $curSQL2 = "SELECT `cur_id` FROM `currency` WHERE `cur_name` = '$dataCode2[price_code]' OR `cur_symbol` = '$dataCode2[price_code]'";
            $dataCur2 = $this->dbF->getRow($curSQL2);
            
            if ($this->dbF->rowCount > 0) {
                return $dataCur2["cur_id"];
            }
        }
