<?php header('Content-type: application/x-javascript');
    ...
    $_w['Order flagged to top successfully!'] = '' ;
    $_w['Order failed to be flagged!'] = '' ;
    $_w['Order flag removed successfully!'] = '' ;
    $_w['Failed to remove flag!'] = '' ;
    $_w[''] = '' ;
    $_w[''] = '' ;
    $_w[''] = '' ;
    $_w[''] = '' ;
...
console.log("aaaa00000");
        ...
        $.ajax({
            type: 'POST',
            url: 'adminorder/order_ajax.php?page=getOrderProductJson',
...
// $('#place_of_delivery').html(html);
 ...
 $("#image_from_url").attr("src", html);
                ...
                $(productHiddenClass).attr("data-val",ui.item.value);
    ...
    $(productId).on('focus : click', function(event) {
    ...
    productStoreJson=function(pid,scaleid,colorid,country){
       ...
       $('#invoice_product_store').attr("readonly","readonly");
        $.ajax({
            type: 'POST',
            url: 'adminorder/order_ajax.php?page=getOrderProductStoreJson',
...
/*                    jAlert('Store not found please try other product. <br> OR Product has no stock avaiable.','Product Inventory Error');*/
    ...
    jsonDataStore = function(data){
        ...
        $("#invoice_product_store").autocomplete({
            ...
            focus: function( event, ui ) {
                ...
                qty= ui.item.qty;
                // qty= "1";
            $("#invoice_qty").attr("data-val",qty).attr("max",qty).val("1");
                price =ui.item.price;
                ...
                $("#invoice_discount").attr("data-val",discount).val("0");
                tempTotalPrice();
            }
        }).on('focus : click', function(event) {
                $(this).autocomplete("search", "");
    ...
    tempTotalPrice = function(){
        qty = parseInt($('#invoice_qtyNew').val());
        updateQty = parseInt($('#invoice_qty').attr('data-update'));
        if(isNaN(updateQty)){
            updateQty = 1;
        }
        price = parseFloat($('#invoice_priceNew').val());
        discount = parseFloat($('#invoice_discountNew').val());
        if(price=='' || isNaN(price)){ price = 0;}
        if(discount=='' || isNaN(discount)){ discount = 0;}

         discount = eval(discount*qty);
        discount = Math.round(discount*100)/100;
        $('#invoice_discountNew').val(discount);
...
function invoiceFormValid(){
    ...
    if(!tempAddProductValid()){
        ...
        if( $("#invoice_product_id__").val() == "" || $("#invoice_product_store").val() == ""
            ){
...
function addListItem() {
    ...
    var pName = $("#invoice_product_id__").val() +" -- "+ $("#invoice_product_scale").val();
    var pNameNew = $("#invoice_product_id__").val() +" - "+ $("#invoice_product_scale").val();
    var price = parseFloat($("#invoice_priceNew").val());
    var discount = parseFloat($("#invoice_discountNew").val());
    var qty =   parseInt($("#invoice_qtyNew").val());
    ...
    if (qty > 0 ) {
        ...
        var item = "<tr id='tr_"+trpid+ "' data-id='"+trpid+"'>"+
            ...
            "<td>"+pName+
                ...
                "<input type='hidden' name='pDiscount_"+trpid+"' value='"+qty*discount+"' class='addedDiscount'/>" +
                "<input type='hidden' name='pName_"+trpid+"' value='"+pNameNew+"' class='addedDiscount__'/>" +
                "<input type='hidden' name='pTotalprice_"+trpid+"' value='"+totalPrice+"' class='addedTotalPrice'/></td>"+
            "<td>"+qty +"</td>"+
            "<td> ("+qty+"*"+price+")-"+qty*discount+"="+totalPrice+" "+priceCode +"</td>"+
            "</tr>";
...
function blankField(){
    //when product add, inserted fields back to blank
    $("#invoice_qtyNew, #invoice_priceNew, #invoice_product_scale, #invoice_product_id__, #invoice_discountNew,#invoice_total_priceNew").val("");
}
...
function finalFormSubmit(){
    ...
    // $('#receiver_name').val(res.fieldnew1);
$('#sender_name').val($('#receiver_name').val());
// $('#receiver_phone').val(res.fieldnew3);
$('#sender_phone').val($('#receiver_phone').val());
// $('#receiver_email').val(res.fieldnew2);
$('#sender_email').val($('#receiver_email').val());
// $('#receiver_city').val(res.fieldnew4);
$('#sender_city').val($('#receiver_city').val());
// $('#receiver_country').val(res.fieldnew5);
$('#sender_country').val($('#receiver_country').val());
// $('#receiver_post').val(res.fieldnew7);
$('#sender_post').val($('#receiver_post').val());
// $('#receiver_address').val(res.fieldnew6);
$('#sender_address').val($('#receiver_address').val());
    ...
    payment = $("#paymentTypeSelect").val();x
    if(payment==""){
...
delOrderInvoice = function(ths){
    ...
    if(secure_delete()){
        ...
        $.ajax({
            type: 'POST',
            url: 'adminorder/order_ajax.php?page=delOrder&id='+id,
            data: { itemId:id }
        }).done(function(data)
        {
            ift =true;
...
flagOrderToTop = function(ths){
    btn=$(ths);
    id=btn.attr('data-id');

    $.ajax({
        url: 'adminorder/order_ajax.php?page=flagOrder&id='+id,
        type: 'POST',
       data: { itemId:id }
    }).done(function(res){
        console.log(res);
        if(res == '1'){
            jAlertifyAlert('<?php echo _js($_e['Order flagged to top successfully!']); ?>', false);
        }else if(res == '0'){
            jAlertifyAlert('<?php echo _js($_e['Order failed to be flagged!']); ?>', false);
        }
    });
}
...
allUser = function(ths){
    btn=$(ths).val();
    // id=btn.attr('data-id');

    $.ajax({
        url: 'adminorder/order_ajax.php?page=allUserData&id='+btn,
        type: 'POST',
         dataType: 'json',
        data: { itemId:btn }
    }).done(function(res){
...
if (res != 0) {
   console.log(res.fieldnew1);
$('#receiver_name').val(res.fieldnew1);
$('#sender_name').val(res.fieldnew1);
$('#receiver_phone').val(res.fieldnew3);
$('#sender_phone').val(res.fieldnew3);
$('#receiver_email').val(res.fieldnew2);
$('#sender_email').val(res.fieldnew2);
$('#receiver_city').val(res.fieldnew4);
$('#sender_city').val(res.fieldnew4);
// $('#receiver_country').val(res.fieldnew5);
// $('#sender_country').val(res.fieldnew5);
$('#receiver_post').val(res.fieldnew7);
$('#sender_post').val(res.fieldnew7);
$('#receiver_address').val(res.fieldnew6);
$('#sender_address').val(res.fieldnew6);
...
removeFlagOrderToTop = function(ths){
    btn=$(ths);
    id=btn.attr('data-id');

    $.ajax({
        url: 'adminorder/order_ajax.php?page=removeFlagOrder&id='+id,
        type: 'POST',
        data: { itemId:id }
    }).done(function(res){
        console.log(res);
        if(res == '1'){
            jAlertifyAlert('<?php echo _js($_e['Order flag removed successfully!']); ?>', false);
        }else if(res == '0'){
            jAlertifyAlert('<?php echo _js($_e['Failed to remove flag!']); ?>', false);
        }
    });
}
...
// final price before order submit
    ...
    console.log(storeCountry,deliverCountry );
    var orderPrice = parseFloat($('.totalPrice ').text());
    
    if(deliverCountry == 'NO'){ 
        priceCode = 'NOK'
    }else if(deliverCountry == 'DK'){
        priceCode = 'DKK'
    } else if(deliverCountry == 'FI'){
        priceCode = 'EURO'
    } else if(deliverCountry == 'DE'){
        priceCode = 'EURO'
    }else if(deliverCountry == 'CH'){
        priceCode = 'CHF'
    } else{
        priceCode = 'SEK'
    }
    
    $('#priceCodeNew').val(priceCode);

     $('.topViewP').fadeIn(500);
     var total = eval(orderPrice+0);
    //  var priceCode =$('#priceCode').val();
    $('.totalPriceModel').text(orderPrice+" "+priceCode);
    $('.totalPriceShipping').text(0+" "+priceCode);
    $('.totalFinal').text(orderPrice+" "+priceCode);

        // $.ajax({
        //     type: "POST",
        //     url: "adminorder/order_ajax.php?page=shippingPrice",
...
function quick_invoice_update(orderid,ths){
    if(secure_delete("Do you want to update?")){
        ...
        $.ajax({
            type: 'POST',
            url: 'adminorder/order_ajax.php?page=quick_invoice_update',
            data: { orderid:orderid,invoice:selected_id,prev_status:pre_status }
        }).done(function(data)
        {
            if(data=='1'){
...
function calculateTotalPrice(){
    let qty = $('#invoice_qtyNew');
    let actPrice = $('#invoice_priceNew');
    let disPrice = $('#invoice_discountNew');
    let totalPrice = $('#invoice_total_priceNew');
    newPrice = Number(actPrice.val())  - Number(disPrice.val());
    newPrice = newPrice * Number(qty.val());
...
$('.paymentTypeFieldset option[value="4"]');

$(".paymentTypeFieldset option[value=4]").attr('selected', 'selected');
