...
$imgs= "<img src='". $img ."' width='50' height='50'/>";
                ...
                $JSON2 .= '{
                        "id" : "' . $id . '",
                        "label" : " <a href=javascript:void(0);>'.$imgs.'<span>' . $name[$defaultLang] . '</span></a>",

                         "value" : "' . $name[$defaultLang] . '",
    ...
    public function getOrderProductStoreJson()
    {
        ...
        if ($this->dbF->rowCount > 0) {
            ...
            $JSON .= trim($JSON2, ',');
             ...
             $storeName = $this->productF->getStoreName(6);
             $price = $this->productF->productTotalPrice($pId, $scleId, $colorId, $customId, $country);
             $discountArray = $this->productF->productDiscount($pId, $countryId);
                if (!empty($discountArray)) {
                    $discount = $discountArray['discount'];
    ...
    public function order_fetch($page)
    {
       ...
       // print_r($page);
        ...
        foreach ($data as $key => $val) {
            ...
            $customer_Name = isset($orderInfo['sender_name']) ? $orderInfo['sender_name'] : '';
...
$sql = "SELECT acc_name,acc_email FROM `accounts_user` WHERE `acc_id` = ?";

$res = $this->dbF->getRow($sql, array($invId));

if($this->dbF->rowCount > 0){

$data['fieldnew1'] =   $res['acc_name'];
$data['fieldnew2'] =   $res['acc_email'];


$sql1 = "SELECT * FROM `accounts_user_detail` WHERE `id_user` = ?";

$res1 = $this->dbF->getRows($sql1, array($invId));
if($this->dbF->rowCount > 0){
foreach ($res1 as $val) {


if ($val['setting_name'] == "phone") {
   $data['fieldnew3'] = $val['setting_val'];
}

if ($val['setting_name'] == "city") {
$data['fieldnew4'] = $val['setting_val'];
}

if ($val['setting_name'] == "country") {
 $data['fieldnew5'] = $val['setting_val'];
}

if ($val['setting_name'] == "address") {
  $data['fieldnew6'] = $val['setting_val'];
}
if ($val['setting_name'] == "post_code") {
$data['fieldnew7'] = $val['setting_val'];
}
